...
import { ButtonSecondary, Button } from '../components/button'
  ...
  return (
    <Layout>
      <Container>
        ...
        >
          <Button message={t('about.button.primary')} link='/about' />
        ...
        <div className='grid grid-flow-dense grid-cols-1 md:grid-cols-3 gap-4 w-full'>
          {Array.from({ length: 3 }).map((_, index) => (
            <UspCard
              key={`usp-${index}`}
              title={t(`about.usp.${index + 1}.title`)}
              description={t(`about.usp.${index + 1}.description`)}
            />
          ))}
        </div>
      </Container>
