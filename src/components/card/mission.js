...
export default function MissionCard({ title, index, description, image, inverted }) {
  return (
    <div
      className={`text-background-950 flex flex-col justify-between items-center font-normal md:max-w-screen-xl my-5 gap-2 ${
        inverted ? 'md:flex-row-reverse' : 'md:flex-row'
      }`}
    >
      <div className='w-full md:w-1/2'>
        <Image loading='lazy' width={500} height={200} alt='Mission illustration' src={image} />
      </div>
      <div
